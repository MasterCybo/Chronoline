package display.components {
	...
	public class DisplayMenuSettings extends ASprite {
		...
		private var _originList:TreeList; // temp
		private var _targetList:TreeList; // temp
		...
		override public function init():* {
			_originList = new TreeList( "originTree" ); // temp
			_targetList = new TreeList( "targetTree" ); // temp
			...
			// Списки
			_originView = new ViewTreeList( _width, _height ).init();
		...
		private function sendPartsRequest():void {
			if ( !_vectItems ) {
				App.httpManager.addRequest( new ReqPartitions(), parsePartitions );
			} else {
				setupList();
			}
		}
		...
		private function procSendEntitiesRequest():void {
			...
			if ( !curItemPart.countChildren ) {
				App.httpManager.addRequest( new ReqPartEntities(( curItemPart.dataObject as MoListPartition ).type ), parseEntities );
			} else {
				procSendEntitiesRequest();
			}
		}
		...
		private function updateSize():void {
			...
			var hh:uint = _btnReady.y - _tfOrigin.height - 10;
			
			_originView.width = _targetView.width = ww;
			_targetView.x = _originView.width + 30;
			
			_originView.height = _targetView.height = hh;
